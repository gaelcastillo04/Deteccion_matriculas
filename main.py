...
# Inicializamos el EasyORC
reader = easyocr.Reader(['es'])

# Array de nombres y matriculas aceptados
...
def procesado_imagen(nombre_archivo):
    ...
    imagen_con_contraste = cv2.convertScaleAbs(imagen_gray, alpha=1.75, beta=-300)  # Alpha contraste y beta brillo
    ...
    # Utilizamos easyocr para detectar caracteres en imagenes
    results=reader.readtext(imagen_con_sharpness, detail=0)  # detail = 0 retorno solo el texto
...
def main():
    ...
    matricula_aprobada=[matricula for matricula in matriculas if matricula in resultado]
    if matricula_aprobada:
        # Si se confirma que la matricula concuerda checas el nombre del alumno
